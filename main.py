...
def get_my_character(request: Request):
    ...
    with engine.connect() as conn:
        ...
        if result and result.class_id is not None:
            ...
            spells_query = select(spells).select_from(spells_j).where(character_spells.c.character_discord_id == user["id"])
...
def get_class_details(class_id: int):
    """Returns features and spells for Step 3 of creation"""
    with engine.connect() as conn:
...
def roll_stats(request: Request, roll_req: RollRequest):
    ...
    with engine.connect() as conn:
        ...
        # STRICT LOCKING: If stats exist, return them. Don't check class_id.
...
def add_species(request: Request, sp: SpeciesCreate):
    ...
    with engine.connect() as conn:
        ...
        conn.execute(insert(species).values(name=sp.name, speed=sp.speed, size=sp.size, description=sp.description))
        conn.commit()
...
def add_class(request: Request, cl: ClassCreate):
    ...
    with engine.connect() as conn:
        ...
        conn.execute(insert(classes).values(name=cl.name, hit_die=cl.hit_die, primary_ability=cl.primary_ability, spellcasting_ability=cl.spellcasting_ability, description=cl.description))
        conn.commit()
...
async def startup_event():
    with engine.connect() as conn:
        if not conn.execute(select(classes)).fetchall():
            class_list = [
                {"name": "Barbarian", "hit_die": 12, "primary_ability": "Strength", "spellcasting_ability": None, "description": "A fierce warrior."},
                {"name": "Bard", "hit_die": 8, "primary_ability": "Charisma", "spellcasting_ability": "Charisma", "description": "An inspiring magician."},
                {"name": "Cleric", "hit_die": 8, "primary_ability": "Wisdom", "spellcasting_ability": "Wisdom", "description": "A priestly champion."},
                {"name": "Fighter", "hit_die": 10, "primary_ability": "Strength/Dexterity", "spellcasting_ability": None, "description": "A master of martial combat."},
                {"name": "Wizard", "hit_die": 6, "primary_ability": "Intelligence", "spellcasting_ability": "Intelligence", "description": "Scholarly magic."}
            ]
        ...
        if not conn.execute(select(spells)).fetchall():
            spell_list = [
        ...
        if not conn.execute(select(class_features)).fetchall():
            cls = {r.name: r.id for r in conn.execute(select(classes)).fetchall()}
            conn.execute(insert(class_features).values(class_id=cls["Fighter"], level_required=1, name="Fighting Style", description="Choose a specialization."))
            ...
            feat_list = [
                {"class_id": cls["Fighter"], "level_required": 1, "name": "Second Wind", "description": "Regain 1d10+Lvl HP once per rest."},
                {"class_id": cls["Fighter"], "level_required": 2, "name": "Action Surge", "description": "Take one additional action."},
                {"class_id": cls["Cleric"], "level_required": 1, "name": "Divine Domain", "description": "Choose your deity's focus."},
                {"class_id": cls["Cleric"], "level_required": 2, "name": "Channel Divinity", "description": "Turn Undead or Domain effect."},
                {"class_id": cls["Wizard"], "level_required": 1, "name": "Arcane Recovery", "description": "Regain some spell slots on short rest."},
            ]
